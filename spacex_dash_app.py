...
def get_pie_chart(entered_site):
    ...
    if entered_site == 'ALL':
        ...
        data = filtered_df.groupby('class')['Flight Number'].count().reset_index()
        fig = px.pie(data, values='Flight Number', 
        names='class', 
        title='Total Launches for {}'.format(entered_site))
        return fig
...
def get_pie_chart(entered_site, playload_range):
    # print(playload_range)
    tmp_df = spacex_df[spacex_df['Payload Mass (kg)']<=playload_range[1]]
    filtered_df = tmp_df[tmp_df['Payload Mass (kg)']>=playload_range[0]]
    if entered_site == 'ALL':
        fig = px.scatter(filtered_df, x='Payload Mass (kg)', 
        y='class', 
        color="Booster Version Category",
        title='Total Launches Scatter')
        return fig
    else:
        # return the outcomes piechart for a selected site
        filtered_df = filtered_df[filtered_df['Launch Site']==entered_site]
